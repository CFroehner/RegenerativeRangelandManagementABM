source("StylizedLandscape/DeprivationDistribution.R")
#This landscape will have 10 layers.
#Among these layers, there are 2 distinct levels: community and individual plots
...
#-Grass growth (rain)
#-Resting (y/n)
#-Grass cover (result of rain and grazing pressure (#cows and whether resting))


#There are also associated data frames: Ranchers
#The 'Ranchers' data frame tracks the community, grazing behavior, 
#cattle buying/selling behavior, and income of each person
...
Comindex<-sample(
  size=N_Communities*Starting_Prop_Conservation, #Get indexes of Cons comms
...
mDepriv<-matrix(
  sample(x=Dep$Dep,size=N_Communities,replace=F),
  nrow=sqrt(N_Communities)) #sample from actual deprivation distribution of AOI
...
rPop[]<-
  ceiling(rPop[]/4) #disaggregation above effectively multiplies the number by 4
#raster::plot(rPop)

#Make ANIMAL Population. Approximation of cattle estimates from AOI
#Old version Now linked to precip below.
#This is not correlated with deprivation as these are not correlated across SA
...
#rAnimal[]<-ceiling(rAnimal[]/4) #the disaggregation above effectively 
...
#First make grass growth without cows. This assumes that all land has same 
# growing potential. Rain is the only determinant. 
#Grass takes number between StartingGrassMin and 1 
source("StylizedLandscape/SpatialAutocorrelationRaster.R")
rT0Rain<-MakeAutoCorMat(
  dummyMatrix = rPlotID,target_I = Spatial_Autocorrelation_Rain) 
#This is normally distributed. We want to bound and make pois

rT0Rain[] <- (rT0Rain[] - min(rT0Rain[])) / 
  (max(rT0Rain[]) - min(rT0Rain[])) * (1 - StartingGrassMin) + StartingGrassMin
...
rAnimal<-raster::disaggregate(rAnimal, fact=2)
rAnimal[]<-ceiling(rAnimal[]/4) #the disaggregation above effectively multiplies 
#the number by 4

#Now we need to assign each plot as resting or not based on whether it's in a 
#conservation area
#Choose random. Later this will be the one with least grass, but not at start bc 
#cows haven't grazed yet
source("StylizedLandscape/GrazedBinarySimulation.R")
#raster::plot(rGrazed) # Returns this raster with 1/4 of the conservation plots 
#labeled as 0

#Now for each community, we need to distribute the grazing evenly across the 
#grazed plots
#so, we need to multiply the grass by 1/0 (whether they're grazed or not), and
#add all the grass in each community up, have animals reduce it and redistribute

rstack<-raster::stack(rCommID,rAnimal,rPlotID,rT0Rain,rGrazed,rCons)
...
df2<-df%>%dplyr::filter(Grazed==1)%>% #Only consider plots that are grazed
  ...
  dplyr::summarize(t0Rain=sum(T0Rain),n=dplyr::n())#get total grass/rain, animals
...
df2 <- merge(df2, CommAnimals, by = "CommID") %>%
  ...
  dplyr::mutate(
    t0Grass = ifelse(
      t0Grass <= 0, 1, t0Grass)) #don't let grass go all the way to 0
...
#Then we invert so that higher numbers are better.
#This number is now considered how much money they have
...
Ranchers <- Ranchers %>%
  ...
  ungroup()%>%
  mutate(
    economic_wellbeing=100-economic_wellbeing)%>% #Making higher numbers "better"
  dplyr::select(-c(rank,wellbeing_raw,mean_raw,shrunk,Depriv))

#Add this to the raster
#Make dataframe from raster
df<-raster::as.data.frame(rstack,xy=T)
df2<-Ranchers%>%group_by(CommID)%>%
  summarise(Mean_economic_wellbeing=mean(economic_wellbeing))
...
PastStockChange<-rnorm(
  n=nrow(Ranchers),mean=0,sd=0.4) #if random (this is the right choice)
...
Ranchers <- Ranchers%>%
  mutate(PastStockChangeProp=PastStockChange,
         ...
         #Above can create negative numbers or 0s in past stock.
         PastStock=ifelse(PastStock<=0,1,PastStock), # Give everyone at least 1
         #Now recalculate the percentages
         PastStockChangeProp=PastStockChangeAnimal/PastStock)%>%
  arrange(CommID,person_id)

#Make a total economic wellbeing used for social learning (their animals & money)
Ranchers<-Ranchers%>%
...
#See here that cows and wealth are associated within community, but not across 
# communities. This again, approximates the actual data from southern africa
